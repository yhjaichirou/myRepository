...
//	}
	 ...
	 * @return
	 */
	public static <T>List<T> analysisIDStr(String idStr,Class<T> bean,JpaRepository<T, Integer> repository){
		List<T> newList = new ArrayList<T>();
		if (StrKit.notBlank(idStr)) {
			for (String id : idStr.split(",")) {
				Optional<T> _members = repository.findById(Integer.parseInt(id));
				if (_members.isPresent()) {
					newList.add(_members.get());
				}
			}
		}
		return newList;
    ...
    public static List<String> getListStringOfSplit(String string,String split){
    	if(isBlank(string)) {
    		return null;
    	}
