...
import com.yhj.pdj.config.LoginConstant;
import com.yhj.pdj.model.po.Token;
...
public class TokenService {
	...
	public String operateToKen(Integer userId) throws SQLException {
		...
		List<User> user = Db.use().find(Entity.create("cat_user").set("id", userId),User.class);
	...
	private void updateToken(Integer userId, String token) throws SQLException {
		Db.use().update(Entity.create("cat_token").set("token", token)
				.set("create_time", new Date()),
				Entity.create("cat_token").set("user_id", userId));
	}
	...
	public void deleteToken(Integer userId) throws SQLException {
		Db.use().del(Entity.create("cat_token").set("user_id", userId));
	}

	public Token getToken(Integer userId) throws SQLException {
		List<Token> tokens = Db.use().findAll(Entity.create("cat_token").set("user_id", userId), Token.class);
		if (tokens.size() > 0) {
