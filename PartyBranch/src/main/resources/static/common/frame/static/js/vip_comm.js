...
// 配置
layui.config({
    base: '/dchip/common/frame/static/js/'  // 模块目录
}).extend({                     // 模块别名
    vip_nav: 'vip_nav'
    , vip_tab: 'vip_tab'
    , vip_table: 'vip_table'
...
//layui.extend({
//	  mod2: '{/}http:l' // {/}的意思即代表采用自有路径，即不跟随 base 路径
//	})
// 主入口方法
layui.use(['layer', 'element', 'util'], function () {
    ...
    window.addTab = function (elem, tit, url) {
        var card = 'card';                                              // 选项卡对象
        var title = tit ? tit :!!elem.children('a')[0]? elem.children('a').html():elem.text();              // 导航栏text
        var src = url ? url : !!elem.children('a')[0]?elem.children('a').attr('href-url'):elem.attr("href-url");      // 导航栏跳转URL
        var id = new Date().getTime();                                  // ID
        if(elem.hasClass("firstList")){
        	var aObj = (elem.next().children(":first").find('a'));
//        	src = aObj.attr("href-url");
//        	title = aObj.text();
        	aObj.children(":first").click();
        }
...
//            	$('#cardtab').load(src);
                element.tabAdd(card, {
                    ...
                    , content: '<iframe class="layui-anim layui-anim-upbit" src="' + src + '" frameborder="0"></iframe>'
                    , id: id
                });
                // 关闭弹窗
                layer.closeAll();
