...
//		 .and().logout().logoutUrl("/admin/login")
		...
		http.
			...
			.and()
				...
				.antMatchers("/error/**","/admin/**","/static/**","/common/**","/js/**","/css/**","/image/**","/views/**","/layuiadmin/**","/test/**")// 设置可以访问的路径
				.permitAll()
				.anyRequest() // 任何请求,登录后可以访问
				.authenticated().and().logout().logoutUrl("/login").logoutSuccessUrl("/login")//此处可以省略，如果此处和登录路径一样会无线重定向
